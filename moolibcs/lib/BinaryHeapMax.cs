// Binary Heap Array Max
//
// (c)2019 Mooflower Ltd.
//
// A standard binary heap array, with max value at root.
...
namespace moo {
	public class BinaryHeapMax<T> where T : IComparable<T> {
		...
		public BinaryHeapMax (int initialCapacity = 0) {
		...
		public BinaryHeapMax (T[] initial, int additionalCapacity = 0, int heapSize = -1) {
		...
		// Raise the node at index up through the tree while it is bigger than
		// its parent.
		...
		// Heapify compares a node with its one child and, if it's bigger,
		...
		// Heapify compares a node with its biggest child and, if it's bigger,
		...
		private void _heapifyWithTwoChildren (int index) {
			...
			int indexBiggestChild;
			int minParent = Size >> 1;
			int indexLeftChild = index << 1;

			while (index <= minParent) {

				// Select the biggest child for comparison
				int indexRightChild = indexLeftChild + 1;
				indexBiggestChild = _heap[indexLeftChild].CompareTo(_heap[indexRightChild]) > 0 ? indexLeftChild : indexRightChild;

				// If the biggest child is <= parent then we're done
				if (_heap[indexBiggestChild].CompareTo(_heap[index]) <= 0) {
					return;
				}

				// Swap the parent with the child
				T t = _heap[index];
				_heap[index] = _heap[indexBiggestChild];
				_heap[indexBiggestChild] = t;

				// Is index a parent of precisely one child?
				index = indexBiggestChild;
